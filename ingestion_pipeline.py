...
class DocxReader:
    """
    קורא docx ומחזיר רשימת פסקאות + אותות מבניים (style/bold) לכל פסקה.

    שינוי מהותי לעומת הגרסה הקודמת: זו הייתה עיוורת לחלוטין לכותרות עמוד
    וכותרות תחתונות (document.paragraphs לעולם לא כולל אותן). אם תאריך
    שיעור יושב ב-header של העמוד - זו בדיוק הבעיה שהתגלתה בבדיקה על קבצים
    אמיתיים בענף המקביל. עכשיו: כותרת העמוד של כל סעיף מוזרקת כ"פסקת דמה"
    בדיוק בגבול הסעיף שבו היא משתנה (לא רק מודבקת בתחילת המסמך), עם
    is_heading_style=True כי מבנית היא מתפקדת בדיוק כמו כותרת.
    """

    @staticmethod
    def read(path: Path) -> tuple[list[dict], dict]:
        """מחזיר (paragraphs, meta). meta משמש את DocumentTypeClassifier."""
        ...
        def inject_header_if_new(section) -> None:
            nonlocal last_header_text
            htext = header_text(section)
            if htext and htext != last_header_text:
                paragraphs.append({
                    "text": htext, "is_heading_style": True, "is_bold": False,
                    "is_section_header": True, "heading_level": None,
                })
                last_header_text = htext

        inject_header_if_new(document.sections[0])
        section_idx = 0
        for p in document.paragraphs:
            ...
            if text:
                ...
                is_heading = style_name.lower().startswith("heading") or "כותרת" in style_name
                paragraphs.append({
                    "text": text,
                    "is_heading_style": is_heading,
                    "is_bold": all_bold,
                    "is_section_header": False,
                    "heading_level": _extract_heading_level(style_name) if is_heading else None,
                })
            if DocxReader._ends_section(p):
                section_idx += 1
                if section_idx < len(document.sections):
                    inject_header_if_new(document.sections[section_idx])

        footer_texts = []
        for s in document.sections:
            ftext = "\n".join(fp.text.strip() for fp in s.footer.paragraphs if fp.text.strip())
            if ftext:
                footer_texts.append(ftext)
        meta = {
            "footer_text": "\n".join(footer_texts),
            "first_header_text": header_text(document.sections[0]),
        }
        return paragraphs, meta

    @staticmethod
    def _ends_section(paragraph) -> bool:
        """True אם הפסקה הזו נושאת sectPr - כלומר היא הפסקה האחרונה של סעיף
        (לא הסעיף האחרון, שה-sectPr שלו חי ברמת ה-body ולא בפסקה)."""
    ...
    def content_hash(paragraphs: list[dict]) -> str:
        """הכותרות המוזרקות לא נכנסות ל-hash - דדופליקציה על תוכן הגוף בלבד,
        כך שקובץ שנשמר מחדש עם תבנית/כותרת מעט שונה עדיין מזוהה כזהה."""
        joined = "".join(p["text"] for p in paragraphs if not p.get("is_section_header"))
        normalized = re.sub(r"\s+", "", joined)
        return hashlib.sha256(normalized.encode("utf-8")).hexdigest()


def _extract_heading_level(style_name: str) -> Optional[int]:
    """'Heading 2' -> 2, 'כותרת 1' -> 1. אם לא ניתן לזהות מספר אבל זו כן
    כותרת (למשל סגנון 'Title') - רמה 1 כברירת מחדל."""
    if not style_name:
        return None
...
class DocumentTypeClassifier:
    """
    שלושה אותות עצמאיים, כפי שנקבע במפרט: (1) כותרת עמוד קצרה ויציבה
    (לוגו/שם שיטה), (2) כותרת תחתונה עם טלפון/מייל/אתר, (3) יחס גבוה של
    פסקאות בסגנון כותרת. לפחות 2 מתוך 3 -> OFFICIAL_METHOD_DOC.

    הערה: הספים המדויקים (80 תווים, יחס 15%) לא נמסרו במפרט המקורי - אלה
    ברירות מחדל סבירות שלי, מוגדרות ב-Config כדי שיהיו קלות לכיוונון. אם
    מסמכי שיטה אמיתיים מסווגים לא נכון - זה המקום הראשון לבדוק.
    """
    _CONTACT_RE = re.compile(
        r"(\b0\d{1,2}[-\s]?\d{7}\b)|([\w.+-]+@[\w-]+\.[\w.-]+)|(www\.[\w-]+\.\w+)|(https?://)"
    )

    @classmethod
    def classify(cls, paragraphs: list[dict], meta: dict, cfg: Config) -> DocumentType:
        signals = 0
        header = meta.get("first_header_text", "")
        if header and len(header) <= cfg.doc_type_header_max_chars:
            signals += 1
        if cls._CONTACT_RE.search(meta.get("footer_text", "")):
            signals += 1
        real_paragraphs = [p for p in paragraphs if not p.get("is_section_header")]
        if real_paragraphs:
            ratio = sum(1 for p in real_paragraphs if p.get("is_heading_style") or p.get("is_heading")) / len(real_paragraphs)
            if ratio >= cfg.doc_type_heading_ratio_threshold:
                signals += 1
        return DocumentType.OFFICIAL_METHOD_DOC if signals >= 2 else DocumentType.LESSON_TRANSCRIPT
...
class Chunker:
    ...
    def chunk_document(self, doc_id: str, content_hash: str, paragraphs: list[dict]
        ...
        def flush(end_index: int) -> None:
            ...
            for part_i, part in enumerate(self._hard_split(text)):
                cid = hashlib.sha256(f"{content_hash}|{buf_start}|{part_i}|{part[:64]}".encode()).hexdigest()[:24]
                chunks.append(Chunk(
        ...
        for i, par in enumerate(paragraphs):
            ...
            if anchor:
                ...
                buf_start = i + 1
                continue
            if par.get("is_section_header"):
                continue  # כותרת עמוד שלא זוהתה כעוגן - מטא-דאטה מבנית, לא תוכן
            state.step()
    ...
    def _hard_split(self, text: str) -> Iterable[str]:
        if len(text) <= self.cfg.chunk_hard_max_chars:
            yield text
            return
        log.warning("chunk באורך %d תווים - פיצול כפוי (קובץ ללא פסקאות?)", len(text))
        for i in range(0, len(text), self.cfg.chunk_target_chars):
            yield text[i:i + self.cfg.chunk_target_chars]


def chunk_by_headings(doc_id: str, content_hash: str, paragraphs: list[dict], cfg: Config) -> list[Chunk]:
    """
    אסטרטגיית chunking למסמכי שיטה רשמיים בלבד: כל התוכן תחת כותרת נכנס
    ל-chunk אחד עם heading_path מלא, לא מפוצל לפי ספירת תווים - פיצול כפוי
    של רעיון פרוצדורלי שלם באמצע היה שובר את המשמעות שלו. כל ה-chunks
    מסומנים TIMELESS (ידע כללי של השיטה, לא קשור לתאריך שיעור).

    תוספת בטיחות שלי (לא הייתה במפרט המקורי): אם קטע תחת כותרת בודדת חורג
    מ-official_doc_hard_cap_chars, כן מפצלים בכוח עם אזהרה - כדי שלא ייווצר
    chunk ענק בלתי מוגבל (למשל טבלה שהודבקה בטעות מתחת לכותרת).
    """
    chunks: list[Chunk] = []
    stack: list[str] = []
    buffer: list[str] = []
    ...
    def flush(end_index: int) -> None:
        ...
        for part_i, part in enumerate(_split_if_oversized(text, cfg)):
            cid = hashlib.sha256(
                f"{content_hash}|{'>'.join(heading_path)}|{part_i}|{part[:64]}".encode()
            ).hexdigest()[:24]
            chunks.append(Chunk(
                chunk_id=cid, doc_id=doc_id, text=part,
                paragraph_range=(buf_start, end_index),
                lesson_number=None, lesson_date=None,
                temporal_status=TemporalStatus.TIMELESS, anchor_distance=0,
                heading_path=heading_path,
            ))
        buffer.clear()

    for i, par in enumerate(paragraphs):
        level = par.get("heading_level")
        if level:
            flush(i - 1)
            while len(stack) >= level:
                stack.pop()
            stack.append(par["text"])
            buf_start = i + 1
            continue
        if par.get("is_section_header"):
            continue  # כותרת עמוד/לוגו - מטא-דאטה מבנית, לא תוכן תחת כותרת
        buffer.append(par["text"])
    flush(len(paragraphs) - 1)
    return chunks


def _split_if_oversized(text: str, cfg: Config) -> Iterable[str]:
    if len(text) <= cfg.official_doc_hard_cap_chars:
        yield text
        return
    log.warning("chunk לפי כותרת חורג מ-%d תווים (בפועל %d) - פיצול בטיחות",
                cfg.official_doc_hard_cap_chars, len(text))
    for i in range(0, len(text), cfg.official_doc_target_chars):
        yield text[i:i + cfg.official_doc_target_chars]
...
class Pipeline:
    ...
    def process_file(self, path: Path) -> DocResult:
        ...
        try:
            ...
            log.error("קריאת קובץ נכשלה %s: %s", path.name, exc)
        ...
        if re.search(r"_processed_\d{4}-\d{2}-\d{2}", path.stem):
            log.warning("⚠️  %s: השם נראה כמו קובץ שכבר עבר עיבוד (יש בו '_processed_TARICH'). "
                        "אם הוא הועתק בטעות בחזרה מ-docs_archive - עדיין יטופל נכון בזכות "
                        "בדיקת התוכן, אבל כדאי לבדוק למה הוא כאן.", doc_id)

        # בדיקה האם המסמך ריק או שלא ניתן היה לחלץ ממנו טקסט
        if not paragraphs or not any(p.get("text", "").strip() for p in paragraphs):
            result.status = "error"
            msg = "המסמך ריק או שלא ניתן היה לחלץ ממנו טקסט (ייתכן שזה PDF סרוק כתמונה)."
            result.warnings.append(msg)
            log.warning("%s: %s", doc_id, msg)
            file_manager.move_to_error(path, self.cfg.error_dir, "מסמך ריק / ללא טקסט", self.cfg.inbox_dir)
            return result
        ...
        if h in self.seen_hashes:
            ...
            log.warning("%s הוא כפילות של מסמך %s - מדלגים", doc_id, self.seen_hashes[h])
            file_manager.move_to_error(path, self.cfg.error_dir, "כפילות של מסמך", self.cfg.inbox_dir)
            return result
        self.seen_hashes[h] = doc_id

        try:
            def _deid_p(p):
                return {**p, "text": self.deid.deidentify(p["text"])}
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                paragraphs = list(executor.map(_deid_p, paragraphs))
        except DeIdentifier.DeIdError as exc:
            result.status = "failed_deid"
            result.warnings.append(str(exc))
            log.error("de-id נכשל עבור %s - עוצר את עיבוד המסמך", doc_id)
        ...
        def _process_chunk(c: Chunk) -> Chunk:
            ...
            c.modality = self.llm.classify_modality(c.text, doc_name=c.doc_id)
            self.verifier.verify_chunk(c)
            return c

        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            chunks = list(executor.map(_process_chunk, chunks))

        result.chunks = chunks

        file_manager.move_to_archive(path, self.cfg.archive_dir, self.cfg.inbox_dir)
        return result
    ...
    def _write_reports(self, results: list[DocResult], works_on: list[dict],
        ...
        manifest = {
            ...
            "files": [{"doc_id": r.doc_id, "status": r.status, "document_type": r.document_type,
                       "n_chunks": len(r.chunks), "n_anchors": len(r.anchors),
    ...
    def _queue_concept_relationships(self, rel_edges: list[dict]) -> None:
        """
