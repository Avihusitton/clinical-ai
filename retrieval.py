...
class Retriever:
    ...
    def answer(self, question: str, current_case_modality: Optional[str] = None) -> str:
        ...
        if not entry_concepts:
            return "אין מספיק מידע בגרף כדי לענות על השאלה הזו - לא נמצא מושג פתיחה מתאים."
        ...
        if not all_paths:
            return f"נמצא מושג פתיחה ('{entry_concepts[0]}') אבל אין קשרים תיאורטיים יוצאים ממנו בגרף. אין מידע מספק."
        ...
        return self._compose(question, current_case_modality, all_paths, exercises)
